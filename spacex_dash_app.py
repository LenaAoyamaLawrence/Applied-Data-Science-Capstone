...
def get_pie_chart(entered_site):
        if entered_site == 'ALL':
            ...
            count = selected_df['class'].value_counts().reset_index(name ='count')
            fig = px.pie(count, values = 'count', names ='class', title = f'Total success launches for site {entered_site}')
            return fig
...
def update_output (selected_site, selected_payload):
       ...
       if selected_site == 'ALL':
            ...
            selected_df = selected_df[selected_df['Payload Mass (kg)'] >= selected_payload[0]]
            selected_df = selected_df[selected_df['Payload Mass (kg)'] <= selected_payload[1]]
